...
function shuffleString(str, seed) {
    let array = str.split(''); // Converte a string em um array de caracteres
    const rng = new Math.seedrandom(seed); // Inicializa o gerador de números pseudoaleatórios com a semente fornecida
    for (let i = array.length - 1; i > 0; i--) {
        const j = Math.floor(rng() * (i + 1));
        [array[i], array[j]] = [array[j], array[i]];
    }
    return array.join(''); // Junta os caracteres de volta em uma string
}
